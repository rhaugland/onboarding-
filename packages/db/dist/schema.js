import { pgTable, pgEnum, uuid, text, timestamp, jsonb, boolean, index, uniqueIndex, } from "drizzle-orm/pg-core";
...
export const onboardingOptions = pgTable("onboarding_options", {
    ...
    baseOptionId: uuid("base_option_id").references(() => onboardingOptions.id, { onDelete: "set null" }),
    skippedSteps: text("skipped_steps").array().notNull().default([]),
    customizeHistory: jsonb("customize_history").notNull().default([]),
    createdAt: timestamp("created_at").defaultNow().notNull(),
}, (table) => ({
    projectBaseCustomizingIdx: index("onboarding_options_project_base_customizing_idx")
...
export const reactions = pgTable("reactions", {
    ...
    uniqueVote: uniqueIndex("reactions_option_voter_idx").on(table.optionId, table.voterName),
